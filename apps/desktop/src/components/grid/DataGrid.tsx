...
function emptyRow(columns: DataGridColumn[]): Row {
  return {
...
function firstEditableField(columns: DataGridColumn[]): string | undefined {
...
function emptyValue(column: DataGridColumn): string | number | boolean {
...
function fieldsFromMessage(message: string, columns: DataGridColumn[], row: Row): string[] {
...
function rowToTsv(row: Row, columns: DataGridColumn[]): string {
 ...
 */
function mergeRows(prev: Row[], nextRows: Row[], editing: EditState | null, saving: boolean): Row[] {
...
/**
 * `boolean` columns render as a checkbox (the value stays a `boolean`). The
 * change goes into the row's draft; it persists with ✓.
 */
 ...
 */
function widthVar(columnId: string): string {
 ...
 */
function displayValue(row: Row, column: DataGridColumn): string {
...
type SelectedCell = { rowId: string; field: string } | null;
...
/**
 * Selected-cell store with per-key notification: moving the selection only wakes
 * the row/column/cell being left and the one being taken, not the ~(visible
 * rows × columns) subscriptions in the body. Notifying all of them cost
 * thousands of snapshots per move and, with the arrow hold at ~25 Hz (see
 * `ARROW_REPEAT_MS`), tens of thousands per second.
 */
interface SelectionStore {
  get: () => SelectedCell;
  set: (next: SelectedCell) => void;
  /** For effects that react to any movement (scrolling, notifying the parent). */
  subscribe: (fn: () => void) => () => void;
  subscribeRow: (rowId: string, fn: () => void) => () => void;
  subscribeField: (field: string, fn: () => void) => () => void;
  subscribeCell: (rowId: string, field: string, fn: () => void) => () => void;
}

/** ` ` as the separator: no `_id` or field name contains one. */
function cellKey(rowId: string, field: string): string {
  return `x:${rowId} ${field}`;
}

function createSelectionStore(): SelectionStore {
  let value: SelectedCell = null;
  const globalListeners = new Set<() => void>();
  const byKey = new Map<string, Set<() => void>>();

  const subscribeKey = (key: string, fn: () => void) => {
    ...
    return () => {
      own.delete(fn);
      if (own.size === 0 && byKey.get(key) === own) byKey.delete(key);
    };
  };

  const keysOf = (sel: SelectedCell, acc: Set<string>) => {
    if (!sel) return;
    acc.add(`f:${sel.rowId}`);
    acc.add(`c:${sel.field}`);
    acc.add(cellKey(sel.rowId, sel.field));
  };
  ...
  return {
    ...
    set(next) {
      const prev = value;
      if (prev?.rowId === next?.rowId && prev?.field === next?.field) return;
      value = next;
      const affected = new Set<string>();
      keysOf(prev, affected);
      keysOf(next, affected);
      for (const key of affected) byKey.get(key)?.forEach((fn) => fn());
    ...
    subscribeRow: (rowId, fn) => subscribeKey(`f:${rowId}`, fn),
    subscribeField: (field, fn) => subscribeKey(`c:${field}`, fn),
    subscribeCell: (rowId, field, fn) => subscribeKey(cellKey(rowId, field), fn),
  };
}
...
function useIsCellSelected(rowId: string, field: string) {
...
function useIsRowActive(rowId: string) {
  const { selection } = useGridUi();
  return useSyncExternalStore(
...
function useIsFieldActive(field: string) {
  const { selection } = useGridUi();
  return useSyncExternalStore(
...
function useOpenCell(rowId: string, field: string) {
...
function useCellState(rowId: string, field: string): number {
...
const ROW_DRAFT = 1;
const ROW_SAVING = 2;
const ROW_HIGHLIGHT = 4;

function useRowActionsState(rowId: string): number {
  const { chrome } = useGridUi();
  return useSyncExternalStore(
    chrome.subscribe,
    useCallback(() => {
 ...
 */
type DraftKind = "" | "new" | "edit";

function useDraftKind(rowId: string): DraftKind {
  const { chrome } = useGridUi();
  return useSyncExternalStore(
    chrome.subscribe,
...
const DRAFT_ROW_CLASS: Record<DraftKind, string> = {
  "": "",
  new: "row-new",
  edit: "row-edit",
};
...
const DRAFT_PINNED_CELL_CLASS: Record<DraftKind, string> = {
  "": "",
  new: "pinned-cell-new",
  edit: "pinned-cell-edit",
};

const EMPTY_CHROME: GridChrome = {
  editing: null,
  saving: false,
  errorFields: new Set(),
  saveError: null,
...
/** A cell's view — checkbox, stars, indented, or the formatted text. */
function CellView({
  row,
  column,
  onToggleBoolean,
...
function RowIndex({ rowId, index }: { rowId: string; index: number }) {
...
>(function DataGrid(
  ...
  const [rows, setRows] = useState<Row[]>(
    () => initialRows ?? [emptyRow(config.columns), emptyRow(config.columns)],
  );
  ...
  const commitEdit = async () => {
    ...
    try {
      ...
      const flagged = fieldsFromMessage(message, config.columns, sanitizedRow);
      setErrorFields(new Set(flagged.length > 0 ? flagged : [firstEditableField(config.columns)].filter(Boolean) as string[]));
      onSaveError?.(message);
      return;
    }
    savingRef.current = false;
   ...
   * used to scan `config.columns` with a `find` on every keypress. */
    ...
    for (const c of config.columns) {
      cols.push(
        columnHelper.accessor((row) => row[c.field], {
          ...
          cell: (ctx) => <GridCellMemo column={c} row={ctx.row.original} columns={config.columns} />,
        }),
      );
    }
    return cols;
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [config.columns, selectionMode]);
  ...
  const textToCopy = (): string | null => {
    ...
    if (copyWholeRowRef.current || (selectionMode === "multiple" && selectedRows.length > 1)) {
      ...
      return source.map((f) => rowToTsv(f, config.columns)).join("\n");
    }
  ...
  const applyPaste = (lines: string[][]) => {
    ...
    if (line.length === 1 && selectedCell) {
      startIdx = config.columns.findIndex((c) => c.field === selectedCell.field);
      if (startIdx < 0) startIdx = 0;
    }
    const { row: next, changed } = applyLineToRow(row, line, startIdx, config.columns);
    if (!changed) return;
