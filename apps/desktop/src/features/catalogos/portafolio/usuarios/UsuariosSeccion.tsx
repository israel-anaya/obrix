...
export function UsuariosSeccion() {
  ...
  const { items, error, loading, create, update, remove, reload } = useCatalogGeneral(USUARIO_API);
            ...
            <div className="flex h-full min-h-0 flex-col overflow-hidden">
              ...
              <div className="min-h-0 min-w-0 flex-1 overflow-hidden">
                <DataGrid
                  ...
                  onAddRow={(fila) => create(filaAUsuarioData(fila))}
                  onEditRow={(fila) => update(fila._id, filaAUsuarioData(fila))}
                  onDeleteRows={remove}
                />
              </div>
            </div>
