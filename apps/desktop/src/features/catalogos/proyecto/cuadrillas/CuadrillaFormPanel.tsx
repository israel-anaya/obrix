...
export function CuadrillaFormPanel({
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="border-b border-border px-3 py-1.5">
        <div className="flex items-center justify-between gap-2">
          <h3 className="truncate text-xs font-semibold text-muted-foreground">
            Ficha{cuadrilla ? ` — ${cuadrilla.clave}` : ""}
          </h3>
          <button
            type="button"
            title="Cerrar"
      ...
      {!datos ? (
        <p className="px-3 py-2 text-xs text-muted-foreground">Selecciona una cuadrilla para ver su ficha.</p>
      ) : (
          ...
          <div className="flex flex-col gap-3">
            ...
            {cuadrilla && (
              <div className="mt-1 grid grid-cols-2 gap-x-3 gap-y-1 border-t border-border pt-2 text-[11px] text-muted-foreground">
                <span>Creado</span>
                <span className="text-right">{formatearFecha(cuadrilla.created_at)}</span>
                <span>Creado por</span>
                <span className="truncate text-right">
                  {nombresPorUsuarioId[cuadrilla.created_by] ?? cuadrilla.created_by}
                </span>
                <span>Actualizado</span>
                <span className="text-right">{cuadrilla.updated_at ? formatearFecha(cuadrilla.updated_at) : "—"}</span>
                <span>Actualizado por</span>
                <span className="truncate text-right">
                  {cuadrilla.updated_by ? (nombresPorUsuarioId[cuadrilla.updated_by] ?? cuadrilla.updated_by) : "—"}
                </span>
              </div>
            )}
          </div>
      ...
      {datos && (
        <div className="flex justify-end gap-2 border-t border-border px-3 py-2">
          <button
            type="button"
            onClick={() => setDatos(cuadrilla ? aCuadrillaData(cuadrilla) : datos)}
