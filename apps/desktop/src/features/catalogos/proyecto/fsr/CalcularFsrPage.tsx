    ...
    return (
      <div className="grid grid-cols-[4fr_1fr] items-center gap-3">
        ...
        <Select value={String(Boolean(valor))} onValueChange={(v) => onCambiar(v === "true")}>
          <SelectTrigger className={cn(FIELD_INPUT_CLASS, "mt-0")}>
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
  ...
  return (
    <div className="grid grid-cols-[4fr_1fr] items-center gap-3">
      ...
      <input
        ...
        className={cn(FIELD_INPUT_CLASS, "mt-0 campo-decimal text-right")}
      />
    </div>
  );
}
    ...
    <div className="flex h-full flex-col">
      <div className="flex items-center justify-end border-b border-border px-4 py-2">
        <div className="flex items-center gap-3">
          ...
          <ActionBar
            actions={[
              { icon: Save, title: guardando ? "Guardando…" : "Guardar cambios", onClick: guardar, disabled: guardando || !!errorModelo },
              { icon: Download, title: "Exportar modelo y parámetros (JSON)", onClick: exportarJson },
              { icon: Upload, title: "Importar modelo y parámetros (JSON)", onClick: importarJson },
            ]}
          />
        </div>
      </div>
