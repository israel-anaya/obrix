...
/** Editor genérico de una variable tipo "rango" — usado tanto para definir el default (`ModeloCalculoPage`) como para capturar sus valores por configuración (`FsrPage`). */
    ...
    <div className="flex flex-col gap-2 rounded-md border border-border bg-muted/30 p-3">
      ...
      {renglones.map((r, i) => (
        <div key={i} className="grid grid-cols-[1.5fr_1fr_1fr_1fr_2rem] items-center gap-2">
          <Input value={r.clasificacion} onChange={(ev) => actualizar(i, { clasificacion: ev.target.value })} placeholder="ej. 1.51 a 2.00 UMA" />
          <Input className="campo-decimal" type="number" step="any" value={r.inferior} onChange={(ev) => actualizar(i, { inferior: Number(ev.target.value) })} />
          <Input
            className="campo-decimal"
            type="number"
            step="any"
            value={r.superior ?? ""}
          ...
          />
          <Input className="campo-decimal" type="number" step="0.00001" value={r.valor} onChange={(ev) => actualizar(i, { valor: Number(ev.target.value) })} />
